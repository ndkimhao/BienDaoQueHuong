...
function FishingGame(domId){
	...
	*/
	...
	(function initFish(){
		Fish.count = 15+attr.goldFish;
		Fish.numPrize = 0;
		//Fish.allowedPrizes = 2;
		Fish.allowedPrizes = attr.goldFish;
		...
		*/
		Fish.prototype.getScore = function(){
			return this.prize ? 100 : ((1+Math.round(9*this.depth))*2);
		}
		...
		*/
		Fish.prototype.reset = function(){
			...
			this.prize = (this.index >= Fish.count-Fish.allowedPrizes && Fish.numPrize < Fish.allowedPrizes && Math.random() > 0.25);
			if (this.prize) Fish.numPrize++;
	...
	*/
	...
	(function initJellyfish(){
		//Jellyfish.count = 5;
		Jellyfish.count = attr.jellyFish;
		...
		return {
			...
			*/
			stopProgram:function(){
				...
				//_outro.show();
				if(_points<0){
					if(!forceDecrementGold(_points*-1)){
						alert("Bạn đã hết thời gian !\nBạn bị phạt "+(_points*-1)+" vàng nhưng không có đủ vàng !");
						bankruptcy();
						return;
					}
				}
                if(_points>0){
                	incrementRecordGold(_points);
					success();
                }
				alert("Bạn đã hết thời gian !\n"+(_points<0?"Bạn bị phạt":"Bạn được thưởng")+" "+(_points<0?(_points*-1):_points)+" vàng !");
			},
			...
			*/
			requestAnimFrame:function(){
				var self=this;
				_frameRate = setTimeout(function(){self.update();}, 28.57142857142857); //1000/35 = 28.57142857142857
			},
			...
			*/
			update:function() {
				...
				this.hook.x > _target.x) {
					var points = this.hook.catchObject() * _multiplier * attr.multiplier;
					_points += points;
					_score.text( _points );
					_net.play();
			...
			*/
			updateTimer:function() {
				...
				if (_seconds > 0) {
					_timeOut = setTimeout(function(){self.updateTimer();}, 1000);
				} else {
					this.endGame();
				}
			},
	...
	*/
	(function init() {
		...
		// Embed sound SWF.
		swfobject.embedSWF("media/fishingSound.swf", soundId, "1", "1", "9.0.45", false, {}, {}, {
			id:soundId,
			name:soundId,
		});
		...
		_sprites = $("<img/>")
			.attr({src:"image/game/fishing/sprites.png"})
			.hide()
			.appendTo(dom)
